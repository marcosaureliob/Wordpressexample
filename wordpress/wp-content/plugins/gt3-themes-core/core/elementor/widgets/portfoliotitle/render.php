...
if($query->have_posts()) {
	...
	<div <?php $widget->print_render_attribute_string('wrapper') ?>>
		...
		while($query->have_posts()) {
			...
			if((bool)$settings['show_category']) {
				...
				if(count($categories)) {
					...
					foreach($categories as $category) {
						...
						$potfolio_categories[] = '<span><a href="'.esc_url($category_link).'">'.$category->name.' <sup>'. ($category->count < 10 ? '0' : '' ). $category->count.'</sup></span></a>';
					}
			...
			$links[] =  '<div class="portfolio_item_wrap" data-index="'.$render_index.'"><a href="'.get_permalink().'">'.get_the_title().'</a>'.
			            (!empty($potfolio_categories) ? ' <span class="divider">/</span> <span class="categories">'.$potfolio_categories.'</span>' : '').'</div>';

			$preview[] = '<img class="image_background" src="'.$src[0].'"  data-index="'.$render_index.'"/>';
