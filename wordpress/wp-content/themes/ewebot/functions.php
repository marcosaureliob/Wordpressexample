...
add_filter('gt3/theme/redux/woocommerce_grid_list', function($options) {
	if (gt3_option('modern_shop') == '1') {
...
add_filter('wpda-builder/registration/settings', function($settings){
...
function gt3_the_breadcrumb(){
    ...
    if(is_front_page() && !is_home()) {
    	...
    	if (is_shop() && (gt3_option('modern_shop') == '1')) {
    		return;
	    }
...
if (!function_exists('gt3_preloader')) {
    function gt3_preloader(){
        ...
        if ((gt3_option('preloader') == '1' || gt3_option('preloader') == true || $mb_preloader == 'custom') && !wp_is_mobile()) {
	        ...
	        if (!empty($preloader_logo['url'])) {
		        $preloader_logo_url = $preloader_logo['url'];
	        }
	        if (!empty($preloader_logo['width'])) {
...
if (!function_exists('gt3_get_page_title')) {
    function gt3_get_page_title($id) {
        ...
        if ($page_title_conditional == 'yes') {
            ...
            $page_title_height = (int)$page_title_height['height'];
...
if (!!gt3_option('disable_elementor_font')) {
...
function gt3_has_shop_on_page(){
	...
	if(!$is_shop && (is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() || is_product() || is_cart() || is_checkout() || is_checkout()) || (function_exists('yith_wcwl_is_wishlist_page') && yith_wcwl_is_wishlist_page())) {
		$is_shop = true;
	}
	if(!$is_shop) {
		$is_shop = false;
